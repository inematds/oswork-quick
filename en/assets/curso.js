...
(function(){
  ...
  document.body.insertAdjacentHTML('afterbegin',
    ...
    '<div class="seltb" id="seltb"><button data-a="hl">highlight</button><button data-a="card">highlight + card</button></div>');
  document.body.insertAdjacentHTML('beforeend',
    '<div class="scrim" id="scrim"></div>'+
    '<aside class="panel" id="jornada"><button class="close" data-close>&#10005;</button><h3>My journey</h3><div class="body">'+
      '<div class="block"><p class="lbl">Course progress</p><div class="pbar"><i id="jpbar"></i></div>'+
      '<p class="stat"><span id="jpct">0</span><small>% read &middot; <span id="jread">0</span>/<span id="jtotal">0</span> sections</small></p></div>'+
      '<div class="block"><p class="lbl">What you can already do</p><ul class="capacidades" id="jcap"></ul></div>'+
      '<div class="block"><p class="lbl">Time</p><div class="timeinvest" id="jtime">'+
        '<div class="ti-block"><span class="ti-num" id="jtinvest">~0min</span><span class="ti-lbl">invested</span></div>'+
        '<div class="ti-block"><span class="ti-num" id="jtrest">~0min</span><span class="ti-lbl">remaining</span></div></div></div>'+
      '<div class="block"><p class="lbl">Cards</p><p class="stat"><span id="jcards">0</span><small>cards</small></p>'+
      '<p class="pendentes-tone" id="jpendtone"></p></div>'+
      '<div class="block"><p class="lbl">Completed practices</p><div class="praticas-check" id="jprat"></div></div>'+
      '<div class="block"><p class="lbl">My highlights</p><div id="jmarks"></div></div>'+
      '<p class="trilha-sintese" id="jsintese" style="display:none"></p>'+
      '<div class="btnrow"><button id="expbtn">export .json</button><button id="impbtn">import</button><button id="resetbtn">reset</button></div>'+
      '<input type="file" id="impfile" accept="application/json" style="display:none"></div></aside>'+
    '<aside class="panel" id="revisar"><button class="close" data-close>&#10005;</button><h3>Review</h3><div class="rev" id="revbody"></div></aside>'+
    '<aside class="panel" id="exercicios"><button class="close" data-close>&#10005;</button><h3>Exercises</h3><div class="exlist" id="exlist"></div></aside>'+
    '<div class="cedit" id="cedit"><button class="close" data-close>&#10005;</button><h3>New card</h3>'+
      '<label>Front (question)</label><textarea id="cefront"></textarea>'+
      '<label>Back (answer)</label><textarea id="ceback"></textarea>'+
      '<p class="cehint">Rewrite the front as a <b>question</b> &mdash; recalling sticks more than recognizing.</p>'+
      '<div class="cerow"><button id="cesave" class="big">save card</button><button id="cecancel">cancel</button></div></div>'+
    '<div class="prefs" id="prefs">'+
      '<div class="prow"><span>Text size</span><span class="pbtns"><button data-pf="size" data-d="-1" aria-label="decrease">A&minus;</button><button data-pf="size" data-d="1" aria-label="increase">A+</button></span></div>'+
      '<div class="prow"><span>Width</span><span class="pbtns"><button data-pf="width" data-d="-1" aria-label="narrower">&minus;</button><button data-pf="width" data-d="1" aria-label="wider">+</button></span></div>'+
      '<div class="prow"><span>Line spacing</span><span class="pbtns"><button data-pf="leading" data-d="-1" aria-label="less">&minus;</button><button data-pf="leading" data-d="1" aria-label="more">+</button></span></div>'+
      '<button class="preset" data-pf="reset">restore default</button></div>'+
    ...
    '<div class="welcome" id="welcome"><button class="close" data-close>&#10005;</button>'+
      '<p class="wk">Welcome</p><h3>How this course works</h3>'+
      '<ul class="wlist">'+
        '<li><b>Increase the text</b> &mdash; tap <b>Aa</b> at the top to adjust size and width; the page was made for that.</li>'+
        '<li><b>Read calmly</b> &mdash; one idea per section, no rush.</li>'+
        '<li><b>Highlight</b> any part and it turns into a <b>review card</b> (you rewrite the question).</li>'+
        '<li><b>Review</b> brings the questions back on its own, right when you need to remember.</li>'+
        '<li><b>Practice now</b>, at the end of each lesson: get hands-on, step by step, always in a safe way.</li>'+
        '<li><b>Journey</b> shows what you can already do. Everything saves by itself in your browser.</li>'+
      '</ul><button class="big" id="wstart">start studying</button></div>');
  ...
  function renderStatcards(){ var box=document.querySelector('#v-trilha .statcards'); if(!box) return;
    ...
    var ts=tempoStats();
    box.innerHTML='<div class="statcard"><span class="sc-num">'+doneAulas+'/'+KEYS.length+'</span><span class="sc-lbl">lessons completed</span></div>'+
      '<div class="statcard"><span class="sc-num">~'+ts.restante+'min</span><span class="sc-lbl">left</span></div>';
  }
  ...
  // ---- tema ----
  var THEMES=[['dark','dark'],['papel','paper'],['sepia','sepia']];
  var themeBtn=$('themebtn');
  function applyTheme(t){ document.documentElement.setAttribute('data-theme',t); var l=(THEMES.filter(function(x){return x[0]===t})[0]||THEMES[0])[1]; if(themeBtn) themeBtn.textContent='theme: '+l; state.prefs.theme=t; save(); }
  ...
  function renderTrilha(){
    KEYS.forEach(function(k){ var a=state.aulas[k], n=readCount(k), total=a.total||1;
      ...
      if(meta) meta.textContent = tempoTxt + (n>=total ? 'completed ✓' : (n>0 ? 'in progress · '+n+'/'+total : total+' sections · open →'));
    });
    renderStatcards();
  }

  function updatePill(){ if(active==='trilha'||!active) return; if($('pill')) $('pill').textContent=readCount(active)+'/'+state.aulas[active].total+' read'; }
  ...
  function refreshReadBtns(){ document.querySelectorAll('.readbtn').forEach(function(b){ var k=aulaOf(b), sec=b.getAttribute('data-read'), done=k&&state.aulas[k].read[sec]; b.classList.toggle('done',!!done); b.textContent=done?'✓ read':'mark as read'; }); }
  ...
  function execCopy(t,btn){ var ta=document.createElement('textarea'); ta.value=t; ta.style.position='fixed'; ta.style.opacity='0'; document.body.appendChild(ta); ta.select(); try{ document.execCommand('copy'); flashBtn(btn,'copied ✓'); }catch(e){} document.body.removeChild(ta); }
  function copyText(t,btn){ try{ if(navigator.clipboard&&navigator.clipboard.writeText){ navigator.clipboard.writeText(t).then(function(){ flashBtn(btn,'copied ✓'); },function(){ execCopy(t,btn); }); return; } }catch(e){} execCopy(t,btn); }
  ...
  document.querySelectorAll('.practice').forEach(function(pr){ var k=aulaOf(pr); if(!k) return; var tasks=state.aulas[k].tasks, boxes=[].slice.call(pr.querySelectorAll('input[data-ptask]')), cnt=pr.querySelector('.pcount');
    function refresh(){ var done=0; boxes.forEach(function(b){ var id=b.getAttribute('data-ptask'); b.checked=!!tasks[id]; if(tasks[id]) done++; }); if(cnt) cnt.textContent=done+'/'+boxes.length+' done'; pr.classList.toggle('done', boxes.length>0&&done===boxes.length); }
  ...
  document.querySelectorAll('.view[data-aula] .col pre').forEach(function(pre){ if(pre.closest('.pcodewrap')||pre.closest('.codewrap')) return; var wrap=el('div','codewrap'); pre.parentNode.insertBefore(wrap,pre); wrap.appendChild(pre); var b=el('button','pcopy'); b.type='button'; b.textContent='copy'; wrap.insertBefore(b,pre); b.addEventListener('click',function(){ copyText(pre.textContent,b); }); });
  ...
  document.querySelectorAll('.termdemo-toggle').forEach(function(b){
    b.addEventListener('click',function(){
      ...
      if(showingOk){ errCmd.classList.add('on'); b.textContent='see the right way'; }
      else { okCmd.classList.add('on'); b.textContent='see the common mistake'; }
    });
  });
  ...
  document.querySelectorAll('.view[data-aula] .step').forEach(function(s){ var rb=s.querySelector('.readbtn'); if(!rb||s.querySelector('.reflect')) return; var k=aulaOf(s), key=rb.getAttribute('data-read'); if(!k||!key) return;
    var d=el('details','reflect'), sum=el('summary'); sum.textContent='Reflect — explain with your own words, or write down a question'; d.appendChild(sum);
    var ta=el('textarea','reflecta'); ta.setAttribute('placeholder','Writing with your own words sticks more than rereading.'); ta.value=state.aulas[k].reflect[key]||''; if(ta.value.trim()) d.classList.add('has'); d.appendChild(ta);
  ...
  document.querySelectorAll('.view[data-aula] .col').forEach(function(col){ var k=aulaOf(col); if(!k||col.querySelector('.recap')) return;
    var box=el('div','recap'); box.innerHTML='<p class="rk">Close the lesson</p><p class="rtext">Review now, with active recall &mdash; then these cards return by themselves at the right time. The lesson track review (button <b>review</b> at the top) mixes questions from <b>all</b> lessons.</p>';
    var actions=el('div','recap-actions');
    var b=el('button','big'); b.textContent='review the cards from this lesson'; b.addEventListener('click',function(){ reviewAula(k); }); actions.appendChild(b);
    if(aulaEls[k]&&aulaEls[k].querySelector('.practice')){
      var b2=el('button','big'); b2.textContent='redo the exercise from this lesson'; b2.addEventListener('click',function(){ resetPractice(k,true); }); actions.appendChild(b2);
    }
  ...
  document.querySelectorAll('.quiz').forEach(function(q){ var ans=q.getAttribute('data-answer'), fb=q.querySelector('.qfb'), done=false;
    q.querySelectorAll('.opt').forEach(function(o){ o.addEventListener('click',function(){ if(done) return; done=true; var kk=o.getAttribute('data-k');
      ...
      if(kk===ans){ fb.innerHTML='<b>Right.</b> '+rfb; } else { o.classList.add('wrong'); fb.innerHTML='<b>Almost.</b> '+(mine?mine+' ':'')+'The correct one is highlighted.'+(rfb?' '+rfb:''); } fb.classList.add('on');
    }); }); });
  ...
  document.querySelectorAll('.view[data-aula]').forEach(function(v){
    ...
    if(sticky&&steps.length&&!v.querySelector('.aulamap')){ var map=el('nav','aulamap'); map.setAttribute('aria-label','Lesson sections');
      steps.forEach(function(s,i){ var h=s.querySelector('h2'), nEl=h&&h.querySelector('.n'), num=nEl?nEl.textContent:(''+(i+1)); var title=h?h.textContent.replace(num,'').trim():('Section '+(i+1));
  ...
  function renderReview(){ var body=$('revbody'); if(ri>=rq.length){ body.innerHTML='<p class="revempty">Nothing to review right now. Come back tomorrow &mdash; or highlight more parts.</p>'; refreshRevn(); return; }
    ...
    var sb=el('button','big'); sb.textContent='show answer'; body.appendChild(sb);
    var gr=el('div','grade'); gr.style.display='none';
    [['again','again'],['good','good'],['easy','easy']].forEach(function(g){ var gb=el('button',''); gb.textContent=g[1]; gb.addEventListener('click',function(){ grade(c,g[0]); save(); ri++; renderReview(); }); gr.appendChild(gb); });
    ...
    var hint=el('p','revhint'); hint.textContent='space reveals · 1 again · 2 good · 3 easy'; body.appendChild(hint);
  }
  ...
  function renderJornada(){ var rd=0,tt=0,cards=0; KEYS.forEach(function(k){ rd+=readCount(k); tt+=state.aulas[k].total; var cs=state.aulas[k].cards; for(var id in cs) cards++; });
    ...
    if($('jpendtone')) $('jpendtone').textContent = due>0 ? ('no rush — '+fmtCount(due)+' waiting for you') : 'nothing waiting for you right now';
    ...
    if(cap){ var caps=[]; KEYS.forEach(function(k){ var a=state.aulas[k], total=a.total||1; if(readCount(k)>=total){ var v=aulaEls[k], p=v&&v.querySelector('.promise'); if(p) caps.push(p.textContent.trim()); } });
      if(!caps.length) cap.innerHTML='<li class="capacidades-empty">no lessons completed yet</li>';
    ...
    var pratBox=$('jprat');
    if(pratBox){ pratBox.innerHTML=''; KEYS.forEach(function(k){ var s=el('span',''); if(praticaDone(k)) s.classList.add('done'); s.textContent=k; s.title='Lesson '+k+(praticaDone(k)?' · practice completed':' · practice pending'); pratBox.appendChild(s); }); }
    ...
    if(sint){ var allDone=KEYS.length>0 && KEYS.every(function(k){ var a=state.aulas[k]; return readCount(k)>=(a.total||1); });
      if(allDone){ sint.style.display='block'; sint.textContent='You completed the entire track — what was on paper is now in your work.'; } else { sint.style.display='none'; }
    }

    var mk=$('jmarks'), all=[]; KEYS.forEach(function(k){ state.aulas[k].marks.forEach(function(m){ all.push(m); }); });
    if(!all.length) mk.innerHTML='<p style="color:var(--muted);font-size:14px">No highlights yet.</p>'; else { mk.innerHTML=''; all.slice().reverse().forEach(function(m){ var d=el('div','mk'); d.textContent='“'+m.text+'”'; mk.appendChild(d); }); }
  }
  ...
  function renderExercicios(){
    ...
    KEYS.forEach(function(k){
      ...
      var h=pr.querySelector('.ph'), title=h?h.textContent.trim():('Lesson '+k);
      var row=el('div','exrow');
      var info=el('div','exinfo');
      var t=el('p','ext'); t.textContent='Lesson '+k+' — '+title; info.appendChild(t);
      var done=praticaDone(k);
      var st=el('p','exstat'+(done?' done':'')); st.textContent=done?'done':'pending'; info.appendChild(st);
      row.appendChild(info);
      var actions=el('div','exactions');
      var go=el('button','exgo'); go.type='button'; go.textContent='go to it'; go.addEventListener('click',function(){ closePanels(); location.hash='aula-'+k; });
      var rs=el('button','exreset'); rs.type='button'; rs.textContent='reset'; rs.addEventListener('click',function(){ resetPractice(k,false); renderExercicios(); });
      actions.appendChild(go); actions.appendChild(rs);
      row.appendChild(actions);
      box.appendChild(row);
    });
    if(!box.children.length) box.innerHTML='<p style="color:var(--muted);font-size:14px">No exercises on this track.</p>';
  }
  ...
  if($('impfile')) $('impfile').addEventListener('change',function(e){ var f=e.target.files[0]; if(!f) return; var r=new FileReader(); r.onload=function(){ try{ var parsed=JSON.parse(r.result); if(!parsed||typeof parsed!=='object'||typeof parsed.aulas!=='object') throw new Error('schema inválido'); state=mergeState(state,parsed); save(); location.reload(); }catch(err){ alert('Invalid file.'); } }; r.readAsText(f); });
  if($('resetbtn')) $('resetbtn').addEventListener('click',function(){ if(!confirm('Reset all course progress?')) return; try{ localStorage.removeItem(CK); }catch(e){} location.reload(); });
