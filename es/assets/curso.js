...
(function(){
  ...
  document.body.insertAdjacentHTML('afterbegin',
    ...
    '<div class="seltb" id="seltb"><button data-a="hl">resaltar</button><button data-a="card">resaltar + tarjeta</button></div>');
  document.body.insertAdjacentHTML('beforeend',
    '<div class="scrim" id="scrim"></div>'+
    '<aside class="panel" id="jornada"><button class="close" data-close>&#10005;</button><h3>Mi trayecto</h3><div class="body">'+
      '<div class="block"><p class="lbl">Progreso del curso</p><div class="pbar"><i id="jpbar"></i></div>'+
      '<p class="stat"><span id="jpct">0</span><small>% leído &middot; <span id="jread">0</span>/<span id="jtotal">0</span> se&ccedil;&otilde;es</small></p></div>'+
      '<div class="block"><p class="lbl">Lo que ya puedes hacer</p><ul class="capacidades" id="jcap"></ul></div>'+
      '<div class="block"><p class="lbl">Tiempo</p><div class="timeinvest" id="jtime">'+
        '<div class="ti-block"><span class="ti-num" id="jtinvest">~0min</span><span class="ti-lbl">invertido</span></div>'+
        '<div class="ti-block"><span class="ti-num" id="jtrest">~0min</span><span class="ti-lbl">restante</span></div></div></div>'+
      '<div class="block"><p class="lbl">Tarjetas</p><p class="stat"><span id="jcards">0</span><small>tarjetas</small></p>'+
      '<p class="pendentes-tone" id="jpendtone"></p></div>'+
      '<div class="block"><p class="lbl">Prácticas completadas</p><div class="praticas-check" id="jprat"></div></div>'+
      '<div class="block"><p class="lbl">Mis resaltados</p><div id="jmarks"></div></div>'+
      '<p class="trilha-sintese" id="jsintese" style="display:none"></p>'+
      '<div class="btnrow"><button id="expbtn">exportar .json</button><button id="impbtn">importar</button><button id="resetbtn">borrar</button></div>'+
    ...
    '<aside class="panel" id="exercicios"><button class="close" data-close>&#10005;</button><h3>Ejercicios</h3><div class="exlist" id="exlist"></div></aside>'+
    '<div class="cedit" id="cedit"><button class="close" data-close>&#10005;</button><h3>Nueva tarjeta</h3>'+
      '<label>Frente (pregunta)</label><textarea id="cefront"></textarea>'+
      '<label>Reverso (respuesta)</label><textarea id="ceback"></textarea>'+
      '<p class="cehint">Reformula el frente como una <b>pregunta</b> &mdash; recordar fija más que reconocer.</p>'+
      '<div class="cerow"><button id="cesave" class="big">guardar tarjeta</button><button id="cecancel">cancelar</button></div></div>'+
    '<div class="prefs" id="prefs">'+
      '<div class="prow"><span>Tamaño del texto</span><span class="pbtns"><button data-pf="size" data-d="-1" aria-label="disminuir">A&minus;</button><button data-pf="size" data-d="1" aria-label="aumentar">A+</button></span></div>'+
      '<div class="prow"><span>Ancho</span><span class="pbtns"><button data-pf="width" data-d="-1" aria-label="más estrecha">&minus;</button><button data-pf="width" data-d="1" aria-label="más ancha">+</button></span></div>'+
      '<div class="prow"><span>Interlineado</span><span class="pbtns"><button data-pf="leading" data-d="-1" aria-label="menos">&minus;</button><button data-pf="leading" data-d="1" aria-label="más">+</button></span></div>'+
      '<button class="preset" data-pf="reset">restaurar el patrón</button></div>'+
    ...
    '<div class="welcome" id="welcome"><button class="close" data-close>&#10005;</button>'+
      '<p class="wk">Bienvenido</p><h3>Cómo funciona este curso</h3>'+
      '<ul class="wlist">'+
        '<li><b>Aumenta la letra</b> &mdash; toca en <b>Aa</b> arriba para ajustar el tamaño y el ancho; la página se hizo para eso.</li>'+
        '<li><b>Lee con calma</b> &mdash; una idea por sección, sin apuro.</li>'+
        '<li><b>Resalta</b> cualquier fragmento y se convierte en una <b>tarjeta de repaso</b> (tú reformulas la pregunta).</li>'+
        '<li><b>Revisar</b> trae las preguntas de vuelta solo, en el momento justo para recordar.</li>'+
        '<li><b>Practica ahora</b>, al final de cada lección: manos a la obra, paso a paso, siempre seguro.</li>'+
        '<li><b>Trayecto</b> muestra lo que ya puedes hacer. Todo se guarda solo en tu navegador.</li>'+
      '</ul><button class="big" id="wstart">empezar a estudiar</button></div>');
  ...
  function renderStatcards(){ var box=document.querySelector('#v-trilha .statcards'); if(!box) return;
    ...
    var ts=tempoStats();
    box.innerHTML='<div class="statcard"><span class="sc-num">'+doneAulas+'/'+KEYS.length+'</span><span class="sc-lbl">lecciones completadas</span></div>'+
      '<div class="statcard"><span class="sc-num">~'+ts.restante+'min</span><span class="sc-lbl">faltan</span></div>';
  }
  ...
  // ---- tema ----
  var THEMES=[['dark','oscuro'],['papel','papel'],['sepia','sepia']];
  ...
  function renderTrilha(){
    KEYS.forEach(function(k){ var a=state.aulas[k], n=readCount(k), total=a.total||1;
      ...
      if(meta) meta.textContent = tempoTxt + (n>=total ? 'concluída ✓' : (n>0 ? 'em andamento · '+n+'/'+total : total+' secciones · abrir →'));
    });
    renderStatcards();
  }

  function updatePill(){ if(active==='trilha'||!active) return; if($('pill')) $('pill').textContent=readCount(active)+'/'+state.aulas[active].total+' leído'; }
  ...
  function refreshReadBtns(){ document.querySelectorAll('.readbtn').forEach(function(b){ var k=aulaOf(b), sec=b.getAttribute('data-read'), done=k&&state.aulas[k].read[sec]; b.classList.toggle('done',!!done); b.textContent=done?'✓ leído':'marcar como leído'; }); }
  ...
  document.querySelectorAll('.practice').forEach(function(pr){ var k=aulaOf(pr); if(!k) return; var tasks=state.aulas[k].tasks, boxes=[].slice.call(pr.querySelectorAll('input[data-ptask]')), cnt=pr.querySelector('.pcount');
    function refresh(){ var done=0; boxes.forEach(function(b){ var id=b.getAttribute('data-ptask'); b.checked=!!tasks[id]; if(tasks[id]) done++; }); if(cnt) cnt.textContent=done+'/'+boxes.length+' hecho'; pr.classList.toggle('done', boxes.length>0&&done===boxes.length); }
  ...
  document.querySelectorAll('.termdemo-toggle').forEach(function(b){
    b.addEventListener('click',function(){
      ...
      if(showingOk){ errCmd.classList.add('on'); b.textContent='ver el correcto'; }
      else { okCmd.classList.add('on'); b.textContent='ver el error común'; }
    });
  });
  ...
  document.querySelectorAll('.view[data-aula] .step').forEach(function(s){ var rb=s.querySelector('.readbtn'); if(!rb||s.querySelector('.reflect')) return; var k=aulaOf(s), key=rb.getAttribute('data-read'); if(!k||!key) return;
    var d=el('details','reflect'), sum=el('summary'); sum.textContent='Reflexiona — explica con tus palabras, o anota una duda'; d.appendChild(sum);
    var ta=el('textarea','reflecta'); ta.setAttribute('placeholder','Escribir con tus palabras fija más que releer.'); ta.value=state.aulas[k].reflect[key]||''; if(ta.value.trim()) d.classList.add('has'); d.appendChild(ta);
  ...
  document.querySelectorAll('.view[data-aula] .col').forEach(function(col){ var k=aulaOf(col); if(!k||col.querySelector('.recap')) return;
    var box=el('div','recap'); box.innerHTML='<p class="rk">Cierra la lección</p><p class="rtext">Revisa ahora, con recordación activa &mdash; después estas tarjetas vuelven solas en el momento justo. La revisión de la ruta (botón <b>revisar</b> arriba) mezcla las preguntas de <b>todas</b> las lecciones.</p>';
    var actions=el('div','recap-actions');
    var b=el('button','big'); b.textContent='revisar las tarjetas de esta lección'; b.addEventListener('click',function(){ reviewAula(k); }); actions.appendChild(b);
    if(aulaEls[k]&&aulaEls[k].querySelector('.practice')){
      var b2=el('button','big'); b2.textContent='volver a hacer el ejercicio de esta lección'; b2.addEventListener('click',function(){ resetPractice(k,true); }); actions.appendChild(b2);
    }
  ...
  document.querySelectorAll('.quiz').forEach(function(q){ var ans=q.getAttribute('data-answer'), fb=q.querySelector('.qfb'), done=false;
    q.querySelectorAll('.opt').forEach(function(o){ o.addEventListener('click',function(){ if(done) return; done=true; var kk=o.getAttribute('data-k');
      ...
      if(kk===ans){ fb.innerHTML='<b>Correcto.</b> '+rfb; } else { o.classList.add('wrong'); fb.innerHTML='<b>Casi.</b> '+(mine?mine+' ':'')+'La respuesta correcta está resaltada.'+(rfb?' '+rfb:''); } fb.classList.add('on');
    }); }); });
  ...
  document.querySelectorAll('.view[data-aula]').forEach(function(v){
    ...
    if(sticky&&steps.length&&!v.querySelector('.aulamap')){ var map=el('nav','aulamap'); map.setAttribute('aria-label','Secciones de la lección');
      steps.forEach(function(s,i){ var h=s.querySelector('h2'), nEl=h&&h.querySelector('.n'), num=nEl?nEl.textContent:(''+(i+1)); var title=h?h.textContent.replace(num,'').trim():('Sección '+(i+1));
  ...
  function renderReview(){ var body=$('revbody'); if(ri>=rq.length){ body.innerHTML='<p class="revempty">Nada para revisar ahora. Vuelve mañana &mdash; o resalta más fragmentos.</p>'; refreshRevn(); return; }
    ...
    var sb=el('button','big'); sb.textContent='mostrar respuesta'; body.appendChild(sb);
    var gr=el('div','grade'); gr.style.display='none';
    [['again','de nuevo'],['good','bien'],['easy','fácil']].forEach(function(g){ var gb=el('button',''); gb.textContent=g[1]; gb.addEventListener('click',function(){ grade(c,g[0]); save(); ri++; renderReview(); }); gr.appendChild(gb); });
    ...
    var hint=el('p','revhint'); hint.textContent='espacio revela · 1 de nuevo · 2 bien · 3 fácil'; body.appendChild(hint);
  }
  ...
  function renderJornada(){ var rd=0,tt=0,cards=0; KEYS.forEach(function(k){ rd+=readCount(k); tt+=state.aulas[k].total; var cs=state.aulas[k].cards; for(var id in cs) cards++; });
    ...
    if($('jpendtone')) $('jpendtone').textContent = due>0 ? ('sin prisa — '+fmtCount(due)+' esperando por ti') : 'nada esperando por ti ahora';
    ...
    if(cap){ var caps=[]; KEYS.forEach(function(k){ var a=state.aulas[k], total=a.total||1; if(readCount(k)>=total){ var v=aulaEls[k], p=v&&v.querySelector('.promise'); if(p) caps.push(p.textContent.trim()); } });
      if(!caps.length) cap.innerHTML='<li class="capacidades-empty">todavía ninguna lección concluida</li>';
    ...
    var pratBox=$('jprat');
    if(pratBox){ pratBox.innerHTML=''; KEYS.forEach(function(k){ var s=el('span',''); if(praticaDone(k)) s.classList.add('done'); s.textContent=k; s.title='Lección '+k+(praticaDone(k)?' · práctica concluida':' · práctica pendiente'); pratBox.appendChild(s); }); }
    ...
    if(sint){ var allDone=KEYS.length>0 && KEYS.every(function(k){ var a=state.aulas[k]; return readCount(k)>=(a.total||1); });
      if(allDone){ sint.style.display='block'; sint.textContent='Concluiste toda la ruta — lo que estaba en el papel ahora está en tu trabajo.'; } else { sint.style.display='none'; }
    }

    var mk=$('jmarks'), all=[]; KEYS.forEach(function(k){ state.aulas[k].marks.forEach(function(m){ all.push(m); }); });
    if(!all.length) mk.innerHTML='<p style="color:var(--muted);font-size:14px">Ningún resaltado todavía.</p>'; else { mk.innerHTML=''; all.slice().reverse().forEach(function(m){ var d=el('div','mk'); d.textContent='“'+m.text+'”'; mk.appendChild(d); }); }
  }
  ...
  function renderExercicios(){
    ...
    KEYS.forEach(function(k){
      ...
      var h=pr.querySelector('.ph'), title=h?h.textContent.trim():('Lección '+k);
      var row=el('div','exrow');
      var info=el('div','exinfo');
      var t=el('p','ext'); t.textContent='Lección '+k+' — '+title; info.appendChild(t);
      var done=praticaDone(k);
      var st=el('p','exstat'+(done?' done':'')); st.textContent=done?'hecho':'pendiente'; info.appendChild(st);
      row.appendChild(info);
      var actions=el('div','exactions');
      var go=el('button','exgo'); go.type='button'; go.textContent='ir hasta ella'; go.addEventListener('click',function(){ closePanels(); location.hash='aula-'+k; });
      var rs=el('button','exreset'); rs.type='button'; rs.textContent='restablecer'; rs.addEventListener('click',function(){ resetPractice(k,false); renderExercicios(); });
      actions.appendChild(go); actions.appendChild(rs);
      row.appendChild(actions);
      box.appendChild(row);
    });
    if(!box.children.length) box.innerHTML='<p style="color:var(--muted);font-size:14px">Ningún ejercicio en esta ruta.</p>';
  }
  ...
  if($('impfile')) $('impfile').addEventListener('change',function(e){ var f=e.target.files[0]; if(!f) return; var r=new FileReader(); r.onload=function(){ try{ var parsed=JSON.parse(r.result); if(!parsed||typeof parsed!=='object'||typeof parsed.aulas!=='object') throw new Error('schema inválido'); state=mergeState(state,parsed); save(); location.reload(); }catch(err){ alert('Archivo inválido.'); } }; r.readAsText(f); });
  if($('resetbtn')) $('resetbtn').addEventListener('click',function(){ if(!confirm('¿Borrar todo el progreso del curso?')) return; try{ localStorage.removeItem(CK); }catch(e){} location.reload(); });
